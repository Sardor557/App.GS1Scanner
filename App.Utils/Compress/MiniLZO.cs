        ...
        private static uint Compress(byte[] src, uint srcstart, uint srcLength, byte[] dst, uint dststart, uint dstlen, uint[] dictNew)
        {
            ...
            else
            {
                ...
                for (; ; )
                {
                    ...
                    if (lpos < srcstart || (offset = lip - lpos) <= 0 || offset > M4_MAX_OFFSET) //if (pos < input || (offset = (uint)(ip - pos)) <= 0 || offset > M4_MAX_OFFSET)
                        literal = true;
                    ...
                    {
                        uint t = lip - lii;//uint t = (uint)(ip - ii);
                        if (t <= 3)
                        {
                    ...
                    {
                        lip--;//--ip;
                        length = lip - lii;//length = (uint)(ip - ii);
                ...
                dstlen = lop - dststart;//dstlen = (uint)(op - output);
                tmp = lin_end - lii;//tmp = (uint)(in_end - ii);
            }
            //}
            if (tmp > 0)
            {
